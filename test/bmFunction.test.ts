...
import { world, loadWorld, setWorldState } from '../src/gameRoom/world.js';
...
describe('草/泥土延迟变化', () => {
    it('被覆盖的草延迟 256 帧后变成泥土', () => {
        setWorldState({ x: 5, y: 5 }, { type: idOfBlock.grass });
        setWorldState({ x: 5, y: 4 }, { type: idOfBlock.stone }); // 上方被石头盖住
    ...
    it('上方无遮挡的草不进入延迟，始终不变', () => {
        setWorldState({ x: 9, y: 5 }, { type: idOfBlock.grass }); // 上方是空气
    ...
    it('延迟期间遮挡被移走，到期后重新验证条件，草保持不变', () => {
        setWorldState({ x: 5, y: 5 }, { type: idOfBlock.grass });
        setWorldState({ x: 5, y: 4 }, { type: idOfBlock.stone });
        grass_and_dirt(idOfBlock.grass, 5, 5); // 满足条件，开始延迟
        setWorldState({ x: 5, y: 4 }, { type: idOfBlock.air }); // 延迟期间盖子被移走
        runFrames(GRASS_DELAY_FRAMES + 100);
        expect(world[5][5]).toBe(idOfBlock.grass);
    });

    it('旁边有草且上方是空气的泥土，延迟后长草', () => {
        setWorldState({ x: 5, y: 5 }, { type: idOfBlock.dirt });
        setWorldState({ x: 6, y: 5 }, { type: idOfBlock.grass }); // 旁边是草
    ...
    it('旁边没有草的泥土，延迟后不变', () => {
        setWorldState({ x: 7, y: 5 }, { type: idOfBlock.dirt });
...
describe('虚草 inviconGrass', () => {
    it('下方是草/泥土时保留，否则消失并生成粒子', () => {
        setWorldState({ x: 5, y: 5 }, { type: idOfBlock.invicon_grass });
        setWorldState({ x: 5, y: 6 }, { type: idOfBlock.grass });
        ...
        setWorldState({ x: 7, y: 5 }, { type: idOfBlock.invicon_grass });
        setWorldState({ x: 7, y: 6 }, { type: idOfBlock.stone });
...
describe('沙子重力 sand_gravity', () => {
    it('下方悬空且高于地形最低点时转为实体方块', () => {
        ...
        setWorldState({ x: 3, y: y }, { type: idOfBlock.sand }); // 下方是空气
    ...
    it('下方有支撑时保持，不高于最低点时转为空气', () => {
        setWorldState({ x: 4, y: 5 }, { type: idOfBlock.sand });
        setWorldState({ x: 4, y: 6 }, { type: idOfBlock.stone });
        expect(sand_gravity(idOfBlock.sand, 4, 5)).toBe(idOfBlock.sand); // 下方有支撑

        setWorldState({ x: 5, y: 0 }, { type: idOfBlock.sand }); // y=0 不高于最低点 lowest_point=0
...
describe('仙人掌/枯灌木 cactus_and_deadBush', () => {
    it('仙人掌下方无支撑时消失并掉落，下方是仙人掌时保持', () => {
        setWorldState({ x: 5, y: 5 }, { type: idOfBlock.cactus });
        setWorldState({ x: 5, y: 6 }, { type: idOfBlock.stone });
        ...
        setWorldState({ x: 6, y: 5 }, { type: idOfBlock.cactus });
        setWorldState({ x: 6, y: 6 }, { type: idOfBlock.cactus });
        expect(cactus_and_deadBush(idOfBlock.cactus, 6, 5)).toBe(idOfBlock.cactus); // 下方是仙人掌，保持
    });

    it('枯灌木下方悬空时消失，有支撑时保持', () => {
        setWorldState({ x: 7, y: 5 }, { type: idOfBlock.deadBush });
        expect(cactus_and_deadBush(idOfBlock.deadBush, 7, 5)).toBe(idOfBlock.air); // 悬空，消失

        setWorldState({ x: 8, y: 5 }, { type: idOfBlock.deadBush });
        setWorldState({ x: 8, y: 6 }, { type: idOfBlock.stone });
...
describe('门 door', () => {
    it('上下两截成对存在时保持，缺一截就消失', () => {
        setWorldState({ x: 5, y: 5 }, { type: idOfBlock.oak_door_bottom });
        setWorldState({ x: 5, y: 4 }, { type: idOfBlock.oak_door_top });
        ...
        setWorldState({ x: 6, y: 5 }, { type: idOfBlock.oak_door_bottom });
        expect(door(idOfBlock.oak_door_bottom, 6, 5)).toBe(idOfBlock.air); // 上半截缺失

        setWorldState({ x: 7, y: 4 }, { type: idOfBlock.oak_door_top });
        expect(door(idOfBlock.oak_door_top, 7, 4)).toBe(idOfBlock.air); // 下半截缺失
    });

    it('开启状态的门同样需要成对', () => {
        setWorldState({ x: 8, y: 5 }, { type: idOfBlock.oak_door_bottom_open });
        setWorldState({ x: 8, y: 4 }, { type: idOfBlock.oak_door_top_open });
        ...
        setWorldState({ x: 9, y: 5 }, { type: idOfBlock.oak_door_bottom_open });
        expect(door(idOfBlock.oak_door_bottom_open, 9, 5)).toBe(idOfBlock.air); // 上半截缺失
    });

    it('世界边缘越界保护，非门方块原样返回', () => {
        setWorldState({ x: 0, y: 0 }, { type: idOfBlock.oak_door_top });
...
describe('门开关 door_openOrClose', () => {
    ...
    it('点击非门方块无变化', () => {
        setWorldState({ x: 6, y: 5 }, { type: idOfBlock.stone });
        mouse.world_x = 6;
        mouse.world_y = 5;
        door_openOrClose();
...
describe('雪草 snowGrass', () => {
    it('上方被遮挡时变成泥土，否则保持', () => {
        setWorldState({ x: 5, y: 5 }, { type: idOfBlock.snowGrass }); // 上方是空气
        expect(snowGrass(idOfBlock.snowGrass, 5, 5)).toBe(idOfBlock.snowGrass);

        setWorldState({ x: 6, y: 5 }, { type: idOfBlock.snowGrass });
        setWorldState({ x: 6, y: 4 }, { type: idOfBlock.stone });
