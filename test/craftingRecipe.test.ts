...
// 验证 recipes 配方数据的单元测试（运行：npx tsx test/craftingRecipe.test.ts）
...
function testNewRecipes(): void {
    for (const expected of expectedNewRecipes) {
        const recipe = findRecipe(expected.outputId);
        check(recipe !== undefined, `配方存在：${expected.name}（outputId=${expected.outputId}）`);
        if (!recipe) {continue;}
        check(sameCells(recipe.cells, expected.cells), `形状正确：${expected.name}`);
        check(recipe.outputCount === expected.outputCount, `单次产出数量正确：${expected.name}（应为 ${expected.outputCount}）`);
    }
}

// 每个新物品应恰好有一个配方产出（防止重复配方互相遮蔽或缺失）
function testNewItemsCoverage(): void {
    ...
    for (const itemId of newItemIds) {
        ...
        check(count === 1, `物品 id=${itemId} 恰好有一个产出配方（实际 ${count} 个）`);
    }
}

// 全部配方的 cells 尺寸必须与 gridWidth/gridHeight 一致
function testGridDims(): void {
    for (const recipe of recipes) {
        check(recipe.cells.length === recipe.gridHeight, `gridHeight 与 cells 行数一致（outputId=${recipe.outputId}）`);
        for (const row of recipe.cells) {
            check(row.length === recipe.gridWidth, `gridWidth 与 cells 列数一致（outputId=${recipe.outputId}）`);
        }
        check(recipe.outputCount >= 1, `产出数量至少为 1（outputId=${recipe.outputId}）`);
    }
}

// 不允许存在形状与材料完全相同的两个配方（先匹配者恒胜，后者被遮蔽）
function testNoDuplicatePatterns(): void {
    const seen = new Set<string>();
    for (const recipe of recipes) {
        const key = JSON.stringify(recipe.cells);
        check(!seen.has(key), `配方形状不重复（outputId=${recipe.outputId}）`);
        seen.add(key);
    }
}

// 所有配方引用的材料与产出都必须在枚举中有定义
function testIdsDefined(): void {
    ...
    for (const recipe of recipes) {
        ...
        check(outputValid, `产出 id 已定义（outputId=${recipe.outputId}）`);
        for (const row of recipe.cells) {
            for (const cell of row) {
                if (cell === null) {continue;}
                const valid = blockIds.has(cell.itemId) || itemIds.has(cell.itemId);
                check(valid, `材料 id 已定义（outputId=${recipe.outputId}，cell itemId=${cell.itemId}）`);
            }
        }
    }
}
...
function main(): void {
    ...
    testNewRecipes();
    testNewItemsCoverage();
    testGridDims();
    testNoDuplicatePatterns();
    testIdsDefined();
    if (failedCount === 0) {
